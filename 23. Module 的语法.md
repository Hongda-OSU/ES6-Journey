...
> 历史上，`JavaScript` 一直没有模块（`module`）体系，无法将一个大程序拆分成互相依赖的小文件，再用简单的方法拼装起来。（其他语言都有这项功能，比如 `Ruby` 的`require`、`Python` 的`import`，甚至就连 `CSS` 都有`@import`）
...
>
> - 不再需要[`UMD`](https://juejin.cn/post/6844903927104667662)模块格式了，将来服务器和浏览器都会支持 `ES6` 模块格式。目前，通过各种工具库，其实已经做到了这一点。
