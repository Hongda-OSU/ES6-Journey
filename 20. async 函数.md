...
> 上面代码的函数`gen`依次读取两个文件，如果写成`async`函数，就是下面这样。
...
>    ```
>
>    上面的代码调用了`asyncReadFile`函数，然后它就会自动执行，输出最后结果。不像 `Generator` 函数，需要调用`next`方法，或者用`co`模块，才能真正执行，得到最后结果。
>
...
> 上面代码指定 `50` 毫秒以后，输出`hello world`。`asyncPrint`函数前面的`async`关键字，表明该函数内部有异步操作。由于`async`函数返回的是 `Promise` 对象，可以作为`await`命令的参数。所以，上面的例子也可以写成下面的形式。
