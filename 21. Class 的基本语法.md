...
> 上面代码定义了一个“类”，可以看到里面有一个`constructor()`方法，这就是构造方法，而`this`关键字则代表实例对象。基本上，`ES6` 的`class`可以看作只是一个语法糖，它的绝大部分功能，`ES5` 都可以做到，新的`class`写法只是让对象原型的写法更加清晰、更像面向对象编程的语法而已。
...
> 注意，新写法定义的属性是实例对象自身的属性，而不是定义在实例对象的原型上面。因为如果原型变了，比如这里的 `_count` 我们做计数处理，那么所有实例的 `_count` 属性都会改变，这就会出现与我们预期不同的结果。
...
> 私有方法和[私有属性](https://zhuanlan.zhihu.com/p/336044542)，是只能在类的内部访问的方法和属性，外部不能访问。这是常见需求，有利于代码的封装，但早期的 `ES6` 不提供，只能通过变通方法模拟实现。
...
class HelloWorld {
  ...
  static #someRandomStuff = 4;

  static #getRandomStuff() {
    return HelloWorld.#someRandomStuff;
  }

  static sayHello() {
    console.log('HELLO WORLD')
    return HelloWorld.#getRandomStuff();
  }
}

HelloWorld.PI // "HELLO WORLD"
HelloWorld.sayHello()
// "HELLO WORLD"
// 4
HelloWorld.#someRandomStuff // 报错
HelloWorld.#getRandomStuff() // 报错
```

> 上面代码中，`#someRandomStuff`是私有属性，`#getRandomStuff()`是私有方法，只能在`HelloWorld`这个类的内部调用，外部调用就会报错。
